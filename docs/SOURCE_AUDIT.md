...
| 출처 | 공식 도메인 | robots.txt | 공식 API | HTML 수집 | PDF 다운로드 | RAG | 모델 학습 | allowed 전환 후보 |
|---|---|---|---|---|---|---|---|---|
| 국가유산포털 | 확인 | 접근 가능, 일부 경로 금지 | 제한된 개별 API 확인 | 조건부 가능 | `unknown` | 문서별 `unknown` | 문서별 `unknown` | 조건부 후보 |
| 한국사데이터베이스 | 확인 | 일반 봇 전체 금지 | 대상 전체 API는 `unknown` | 불가 | 자동 다운로드 불가 | 자동 수집 불가 | 자동 수집 불가 | 제외 |
| 목포시청·목포문화관광 | 확인 | HTTP 400, 규칙 확인 실패 | `unknown` | `unknown` | `unknown` | 문서별 `unknown` | 문서별 `unknown` | 제외 |
| 국가기록원 | 확인 | 접근 가능, 금지 규칙 없음 | 확인 | 조건부 가능 | 대표 공개 첨부 확인 | 문서별 `unknown` | 문서별 `unknown` | 조건부 후보 |
| 공공누리 | 확인 | 접근 가능, 일부 봇/경로 규칙 | 공개 API 문서 `unknown` | 조건부 가능 | 대표 공개 PDF 확인 | 표시 유형별 가능 | 표시 유형별 가능 | 조건부 후보 |
| OAK | 확인 | 접근 가능, 일부 경로 금지 | 확인 | 조건부 가능 | 대표 공개 PDF 확인 | 문서별 `unknown` | 문서별 `unknown` | 조건부 후보 |
| KCI | 확인 | 일반 봇 전체 금지 | 확인 | 불가 | 자동 다운로드 불가 | 자동 수집 불가 | 자동 수집 불가 | 제외 |

`collection_status`는 이번 검토에서 변경하지 않았다.
...
**allowed 전환 후보:** 제외. robots.txt 정책이 바뀌거나 국사편찬위원회의 명시적 허가를 받기 전에는 HTML/PDF 수집기를 허용하지 않는다. 승인받은 API가 목적과 범위에 맞는 경우 API 전용 출처로 별도 검토한다.
...
**allowed 전환 후보:** 조건부 후보. 우선 공식 OpenAPI의 메타데이터만 수집하고, 원문은 공개 상태와 공공누리 유형이 명확한 항목만 별도 허용 목록에 넣는 방식이 적합하다.
...
**allowed 전환 후보:** 조건부 후보. 승인받은 Open API로 메타데이터를 수집하고, 원문은 라이선스가 명시된 논문만 허용하는 두 단계 수집기로 제한한다.
...
**allowed 전환 후보:** HTML/PDF 수집기는 제외. KCI의 승인을 받은 Open API에 한해 메타데이터 전용 수집기로 별도 후보가 될 수 있다.

## allowed 전환 제안

현재 상태를 곧바로 `allowed`로 바꿀 출처는 없다. 다음의 좁은 범위만 후속 검토 후보로 제안한다.

1. **국가기록원 API 메타데이터**: 인증키 발급과 API 약관 확인 후, 원문이 아닌 검색 메타데이터만 수집
2. **OAK 승인 API 메타데이터**: API 신청 승인 후, 금지된 `/openapi/` HTML 경로를 크롤링하지 않고 API만 호출
3. **공공누리 사전 승인 URL 목록**: 공공누리 유형과 표시 문구를 개별 검증하고 검색 결과 페이지는 수집하지 않음
4. **국가유산포털 허용 HTML 상세 경로**: `/heri/unified/`를 제외하고 저작권 정책과 자료별 표시를 추가 확인한 URL만 등록

한국사데이터베이스와 KCI의 일반 HTML/PDF 수집, robots.txt를 확인하지 못한 목포시청 수집은 allowed 후보가 아니다.
